...
Changed
- GitHub 드롭다운에서 선택한 `origin/<branch>`가 아직 로컬에 없으면 안전한 refspec으로 fetch한 뒤 비교하도록 개선
- 큰 심볼 diff는 LCS 행렬이 과도하게 커지기 전에 제한된 fallback hunk로 전환

Docs
- 브랜치 심볼 비교 콘솔의 제품 방향과 승인된 3-pane 디자인 기준 문서 추가

Tests
- Git 어댑터, GitHub 브랜치 대체 경로, 원격 ref 자동 fetch, C 심볼 추출, 세션 저장, 비교 리포트, 큰 심볼 diff, Playwright 스모크 테스트 추가
